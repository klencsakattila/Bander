...
describe('Bander – nyitóoldal', () => {
  ...
  it('TC-FE-001 – Nyitóoldal betöltése', () => {
    ...
    cy.get('h1').contains(/Bander - musician finder/i).should('be.visible');
  });
  ...
  it('TC-FE-003 – New Artists blokk', () => {
    ...
    cy.get('[href="/artist/1"] > .artist-card').should('have.length.at.least', 1);
  });

  it('TC-FE-004 – Bands előnézet', () => {
    cy.contains(/Bands/i).scrollIntoView();
    cy.get('[href="/band/19"] > .band-item > h4').should('have.length.at.least', 1);
  });

  it('TC-FE-005 – Upcoming events blokk', () => {
    cy.contains(/Upcoming events/i).scrollIntoView();
    cy.get('.events-grid > :nth-child(1)').should('have.length.at.least', 1);
  });
